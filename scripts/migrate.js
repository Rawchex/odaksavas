// Run with: node --env-file=.env scripts/migrate.js or called on server startup
'use strict';
...
function runMigrations(cb) {
  ...
  db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS video_upload_logs (
    ...
    db.run(`CREATE INDEX IF NOT EXISTS idx_video_upload_logs_user_date ON video_upload_logs(user_id, created_at)`);
